...
class ForumEntry {
    ...
    static function getList($type, $parent_id)
    {
        ...
        switch ($type) {
            ...
            case 'list':
                ...
                // for an area ordered by sticky-thread property and the mkdate of their latest posting
    ...
    /**
     * Get info if thread is marked as sticky thread
     * 
     * @param string $topic_id
     * 
    ...
    static function getStickyInfo($topic_id)
    {
        
        
            $stmt = DBManager::get()->prepare("SELECT sticky
                FROM forum_entries
                WHERE topic_id = ?");
    ...
    static function insert($data, $parent_id) 
    {
        ...
        $stmt = DBManager::get()->prepare("INSERT INTO forum_entries
            (topic_id, seminar_id, user_id, name, content, mkdate, latest_chdate,
                chdate, author, author_host, lft, rgt, depth, anonymous, sticky)
        ...
        $stmt->execute(array($data['topic_id'], $data['seminar_id'], $data['user_id'],
            ...
            $constraint['rgt'], $constraint['rgt'] + 1, $constraint['depth'] + 1, 0, $data['sticky']));
 ...
 /**
     * update the passed topic as sticky
     *
     * @param type $topic_id the id of the topic to update
     * @param type sticky is 1 if thread should be sticky thread
     *
     * @return void
     */
    static function update_sticky($topic_id,$sticky_value)
    {
        $topic = ForumEntry::getConstraints($topic_id);
